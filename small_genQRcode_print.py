...
def add_margins(qr_code,datadf,i,size_type):
    ...
    M,N = qr_code.size
    qr_code=imgAddFont_bottom(qr_code,M,N,str(datadf['资产编码'][i]),datadf['资产名称'][i],datadf['财政资产编号'][i],note=datadf['备注'][i],size_type=size_type)
    return qr_code
...
if __name__=="__main__":
    ...
    btn3 = tk.Button(frm, text = '开始生成',command=lambda: gen_qrcode(entry1.get(), size_type.get()))
